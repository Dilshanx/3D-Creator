// components/3d/Create/Model3DCreator.jsx
// ... (imports and setup from previous response part 1) ...
...
import { TextureLoader } from "three"; // For export texture loading
...
import {
  MainScene,
  CameraController,
  createMeshFromShape, // This will now also handle applying textures for export
  exportToGLB,
} from "./SceneElements";
...
export default function Model3DCreator() {
  ...
  const shapeTextureFileInputRefs = useRef({}); // For general shape textures
  const textTextureFileInputRefs = useRef({}); // Specifically for text object textures
  ...
  const addShape = useCallback(
    (geometryType, options = {}) => {
      ...
      const newShapeBase = {
        ...
        textureProps: { ...initialTextureProps }, // For general shape
      };
      let specificProps = {};
    ...
    try {
      const serializableShapes = shapes.map((s) => {
        ...
        if (s.type === "text")
          ...
          }; // GLBs don't use shape.textureProps
          ...
          }; // ImagePlanes don't use shape.textureProps
        return {
          ...baseShape,
          color: s.color,
          material: s.material,
  ...
  const exportStaticGLBFile = useCallback(async () => {
    ...
    try {
      ...
      const textureLoaderForExport = new TextureLoader(); // For export
      ...
      const meshPromises = shapes.map(async (shapeData) => {
        if (shapeData.type === "importedGLB") {
          /* ... (same GLB handling) ... */ const gltfObjectData =
            loadedGltfObjects[shapeData.id];
          if (gltfObjectData && gltfObjectData.scene) {
        ...
        // For text and other primitives, create them with textures
        let mesh;
        if (shapeData.type === "text") {
          ...
          const texProps = shapeData.textTextureProps || initialTextureProps; // Use text specific textures
          if (texProps.mapUrl) {
          ...
          // Add other maps if needed for text (roughnessMap, metalnessMap, etc.) from texProps

          mesh = new THREE.Mesh(textGeo, material);
        } else {
          // Use createMeshFromShape for primitives but then override material with textures
          mesh = await createMeshFromShape(shapeData); // This creates basic mesh
          if (
            mesh &&
            shapeData.textureProps &&
            shapeData.type !== "imagePlane"
          ) {
            // Image planes handle their own texture
            const texProps = shapeData.textureProps;
            const newMaterial = mesh.material.clone(); // Clone to avoid modifying shared materials
            if (texProps.mapUrl) {
            ...
            // If base color is used and no map, ensure it's set (might be overwritten by map logic)
  ...
  const bakeAndExportAnimatedGLB = useCallback(async () => {
    // Similar texture logic needs to be applied here
    if (shapes.length === 0) {
      /* ... */ return;
    }
    if (isBaking) {
    ...
    try {
      ...
      const meshCreationPromises = shapes.map(async (shapeData) => {
        ...
        if (shapeData.type === "importedGLB") {
          ...
          /* ... (same text geometry + Z-scaling as in exportStaticGLBFile) ... */
          let fontToUse = helvetikerFontForExport;
          if (!fontToUse) {
            try {
              fontToUse = await new Promise((resolve, reject) => {
          ...
          targetObjectForAnimation = await createMeshFromShape(shapeData); // Base mesh
          if (
  ...
  const handleJsonFileImport = useCallback(
    (event) => {
      /* ... (same as before, ensure textureProps and textTextureProps are loaded if present in JSON) ... */ if (
        isBaking
      ) {
        alert("Cannot import while baking.");
        return;
  ...
  const processAndAddImageFile = useCallback(
    (file) => {
      /* ... (same) ... */ if (!file.type.startsWith("image/")) {
  ...
  const handleDropOnCanvas = useCallback(
            ...
            try {
              ...
              loader.parse(
                ...
                (error) => {
                  ...
                  alert("Error parsing dropped GLB/GLTF: " + error.message);
                }
              );
            } catch (error) {
              console.error("Error processing dropped GLB/GLTF:", error);
              alert("Error processing dropped GLB/GLTF: " + error.message);
            }
          };
          reader.onerror = (e) => {
            console.error("Error reading dropped GLB/GLTF file:", e);
            alert("Error reading dropped GLB/GLTF file.");
          };
          reader.readAsArrayBuffer(file);
    ...
    [isBaking, addImportedShape, processAndAddImageFile, handleJsonFileImport]
  );
  const alignAllShapes = useCallback(
    (axis, reference = "average") => {
      /* ... (same) ... */ if (isBaking || shapes.length < 2) return;
      saveState();
      let targetValue;
  ...
  const alignSelectedShapeToOrigin = useCallback(
    (axis) => {
      /* ... (same) ... */ if (isBaking || !selectedShapeId) return;
      saveState();
      setShapes((prevShapes) =>
        prevShapes.map((s) => {
          if (s.id === selectedShapeId) {
            ...
            if (axis === "y") {
              ...
              if (s.scale && s.scale[1]) {
                ...
                else if (s.type === "importedGLB" || s.type === "customMesh")
                  yOffset = 0;
    ...
    (shapeId, mapType, event, isTextSpecific = false) => {
      ...
      if (file) {
        ...
        reader.onload = (e_reader) => {
          ...
          // saveState(); // Trigger history save after texture update
        };
        reader.readAsDataURL(file);
      }
  ...
  ); // Add saveState if you want immediate history push
  ...
  const handleClearTexture = useCallback(
    (shapeId, mapType, isTextSpecific = false) => {
      ...
      // saveState(); // Trigger history save
    },
    [
      /*saveState*/
    ]
    ...
    <TooltipProvider>
      ...
      >
        ...
        <input
          ...
          style={{ display: "none" }}
        />
        {/* Hidden file inputs for textures - one for shape, one for text */}
        ...
        {Object.keys(initialTextureProps).map(
          (
            mapType // Simplified text texture slots for now
          ) =>
        ...
        <div className='flex flex-grow min-h-0'>
          ...
          <PropertiesPanel
            ...
            addShape={addShape}
            handleTextureUpload={handleTextureUpload} // Pass texture handlers
            handleClearTexture={handleClearTexture} // Pass texture handlers
            shapeTextureFileInputRefs={shapeTextureFileInputRefs} // Pass refs
            textTextureFileInputRefs={textTextureFileInputRefs} // Pass refs
          />
        </div>
        {selectedShape &&
          selectedShape.type === "importedGLB" &&
