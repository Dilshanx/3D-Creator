...
// Popular 2D Shape creation utilities
import {
  createHeartShape,
  createStarShape,
  createCrownShape,
...
let useFrame, useLoader, useThree, extend;
let OrbitControls, TransformControls, Grid, Environment, Text3D;
...
const globalFontLoader = new FontLoader();
const FONT_PATH = "/fonts/helvetiker_regular.typeface.json"; // Ensure in public/fonts/

globalFontLoader.load(
  FONT_PATH,
  (font) => {
    helvetikerFont = font;
    console.log("SceneElements: Helvetiker font loaded for exports.");
  },
  undefined,
  (err) => {
    console.error(
      "SceneElements: Failed to load Helvetiker font for exports:",
      err
    );
  }
);
...
export function exportToGLB(scene, filename = "model.glb") {
  // ... (exportToGLB logic remains the same as previously provided correct version)
  if (!THREE || !GLTFExporter) {
    alert("THREE.js or GLTFExporter is not available for GLB export.");
    console.error("THREE.js or GLTFExporter not available.");
    return;
...
export const createMeshFromShape = async (shapeData) => {
  // ... (createMeshFromShape logic remains the same, ensuring it handles 'text' with TextGeometry and helvetikerFont)
  if (!THREE) {
    console.error("THREE.js is not available.");
    return null;
  }
  try {
    ...
    switch (shapeTypeProp) {
      ...
      case "text":
        if (!helvetikerFont) {
          console.warn(
            "Export: Helvetiker font not preloaded. Attempting to load on-demand..."
          );
          try {
            helvetikerFont = await new Promise((resolve, reject) => {
              globalFontLoader.load(FONT_PATH, resolve, undefined, reject);
            });
          } catch (e) {
            console.error("Export: Dynamic font load failed:", e);
        ...
        geometry = new TextGeometry(shapeData.text || "3D", {
          font: helvetikerFont,
          size: shapeData.textSize || 0.5,
          height: extrudeDepth,
          curveSegments: 12,
          bevelEnabled: true,
          bevelThickness: (shapeData.textSize || 0.5) * 0.04,
          bevelSize: (shapeData.textSize || 0.5) * 0.04,
          bevelOffset: 0,
          bevelSegments: 3,
        });
        geometry.center();
    ...
    if (shapeTypeProp === "imagePlane") {
      ...
      /* ... same primitive/text material logic ... */
      const color = new THREE.Color(shapeData.color || "#ffffff");
      const pbrProps = {
        color,
        roughness:
    ...
    mesh.name = `shape_${shapeData.id}_${shapeTypeProp}_${
      ...
      shapeData.shapeType ||
      (shapeTypeProp === "text"
        ? shapeData.text?.substring(0, 10) || "Text"
        : "")
    }`;
    mesh.castShadow = shapeTypeProp !== "imagePlane";
    mesh.receiveShadow = true;
...
function Shape({
  ...
  onClick,
  text,
  textSize, // For text type
  shapeType,
  shapeSize = 1,
  extrudeDepth = 0.2, // For customExtruded and text height
  objectRef,
  animation,
  isAnimating,
  isTransformDragging,
  ...
  // ... (Shape component logic remains the same, using Text3D for 'text' type)
  const localMeshRef = useRef(null);
  const orbitAngleRef = useRef(Math.random() * Math.PI * 2);
  ...
  useEffect(() => {
    if (imageTexture) imageTexture.colorSpace = THREE.SRGBColorSpace;
  }, [imageTexture]);
  ...
  }, [type, materialType, color, roughness, metalness, imageTexture]);
  ...
  if (type === "text") {
    ...
    return (
      ...
      >
        <Text3D
          ...
          font={FONT_PATH}
          size={textSize || 0.5}
          height={extrudeDepth || 0.1}
          curveSegments={12}
          bevelEnabled
          bevelThickness={(textSize || 0.5) * 0.04}
          bevelSize={(textSize || 0.5) * 0.04}
          bevelOffset={0}
          bevelSegments={3}
          name={objectName}
          castShadow
    ...
    /* ... same primitive/image plane logic ... */
    const proceduralGeometry = useMemo(() => {
      switch (type) {
        case "box":
          return <boxGeometry args={[1, 1, 1]} />;
    ...
    }, [type, shapeType, shapeSize, extrudeDepth, planeWidth, planeHeight]);
    return (
      <mesh
        ref={localMeshRef}
        onClick={(e) => {
...
export function MainScene({
  ...
  }, [selectedShapeId, selectedObjectInternalRef.current]); // transformObject removed from deps here to avoid loop
