...
function LoadedTexturesMaterial({
  ...
  const materialProps = useMemo(() => {
    const hasMapTexture = textures && textures.map;
    const matColor = hasMapTexture ? 0xffffff : color || "#ffffff";
...
// #############################################################
// ### CORRECTED SHAPE COMPONENT STARTS HERE ###
// #############################################################
...
function Shape({
  ...
  useFrame((state, delta) => {
    ...
    // ----- FIX APPLIED HERE -----
    // The condition `type !== "importedGLB"` was removed from the if statement
    // to allow this animation logic to run for GLB models.
    if (
      isAnimating &&
      !isTransformDragging &&
      animation &&
      ...
      // This block will now animate GLBs as well as procedural shapes.
      // NOTE: This will override the GLB's internal animations if both are active.
      const effectiveSpeed = (animation.speed || 1) * delta;
      let currentP = livePosition.current.clone();
      let currentR = liveRotation.current.clone();
      switch (animation.type) {
      ...
      // If not animating, ensure the object is at its defined position/rotation
      targetRef.position.copy(livePosition.current);
      targetRef.rotation.copy(liveRotation.current);
    }
      ...
      >
        ...
        >
          ...
          <LoadedTexturesMaterial
            ...
            roughness={roughness}
            metalness={metalness}
            textureProps={currentMaterialAndTextureProps}
          />
        </Text3D>
      </Suspense>
    );
...
// #############################################################
// ### CORRECTED SHAPE COMPONENT ENDS HERE ###
// #############################################################
      ...
      if (
        ...
        const setupAction = (clip, clipIndex) => {
          ...
          if (animationPlaybackState === "playing") {
            ...
            action.play();
            action.paused = true;
            if (absoluteTime === 0 && !action.isRunning()) mixer.setTime(0);
          } else {
            action.stop();
            if (absoluteTime === 0 && !action.isRunning()) mixer.setTime(0);
          }
      ...
      {shapes.map((shapeData) => {
        ...
        return (
          <Shape
            ...
            gltfModelScene={gltfObjectData?.scene}
            animation={shapeData.animation}
            isAnimating={isAnimating} // Pass global animation state to all shapes
            onClick={onShapeClick}
            objectRef={
              shapeData.id === selectedShapeId
                ? selectedObjectInternalRef
