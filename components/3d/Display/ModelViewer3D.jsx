...
// above code is workingand fixed full screen issue lets fix 3d text issue
...
const ModelViewer3D = () => {
  ...
  const handleExportGLB = useCallback(async () => {
    ...
    if (isTextVisible && current3DText && settings.textFontUrl) {
      ...
      if (fontToUse) {
        const exportTextSize = saneNumber(settings.textSize, 0.5);
        const exportTextDepthValue = saneNumber(settings.textDepth, 0.05);
        const useBevels = true; // As in original code, Text3D default and previous export code

        const textGeom = new TextGeometry(current3DText, {
          font: fontToUse,
          size: exportTextSize,
          height: exportTextDepthValue, // This 'height' param in TextGeometry is extrusion depth
          curveSegments: 12,
          bevelEnabled: useBevels,
          bevelThickness: useBevels
            ? saneNumber(0.02 * (exportTextSize / 0.5), 0.01)
            : 0,
          bevelSize: useBevels
            ? saneNumber(0.01 * (exportTextSize / 0.5), 0.005)
            : 0,
        });

        // FORCE CORRECT Z-SCALING
        textGeom.computeBoundingBox();
        const actualZDepth =
          textGeom.boundingBox.max.z - textGeom.boundingBox.min.z;
        const expectedZDepth = exportTextDepthValue;
        ...
        if (
          ...
          zScaleFactor = 1.0; // No scaling needed if both are zero
        } else if (actualZDepth === 0 && expectedZDepth !== 0) {
          console.warn(
            "[Export Text] Z-depth warning: Actual Z-depth is 0, but expected Z-depth is not. Z-scaling may not work as intended."
          );
        }

        console.log("[Export Text] Z-depth correction:");
        console.log(
          "  - Expected Z-depth (from settings.textDepth):",
          expectedZDepth
        );
        console.log(
          "  - Actual Z-depth (pre-scale, from TextGeometry bounding box):",
          actualZDepth
        );
        console.log("  - Z-scale factor:", zScaleFactor);

        // Apply Z-scaling to fix the depth
        if (Math.abs(zScaleFactor - 1.0) > 0.0001 && isFinite(zScaleFactor)) {
          textGeom.scale(1, 1, zScaleFactor);
        }

        // Re-compute bounding box after scaling
        textGeom.computeBoundingBox();
        console.log("[Export Text] After Z-scaling (if applied):");
        console.log(
          "  - New Z-range:",
          `${textGeom.boundingBox.min.z.toFixed(
            4
          )} to ${textGeom.boundingBox.max.z.toFixed(4)}`
        );
        console.log(
          "  - New Z-depth (post-scale, from TextGeometry bounding box):",
          (textGeom.boundingBox.max.z - textGeom.boundingBox.min.z).toFixed(4)
        );

        // Now do the manual X,Y centering (as per user request, Z is not re-centered here)
        const centerOffsetX =
          -0.5 * (textGeom.boundingBox.max.x + textGeom.boundingBox.min.x);
        const centerOffsetY =
          -0.5 * (textGeom.boundingBox.max.y + textGeom.boundingBox.min.y);
        textGeom.translate(centerOffsetX, centerOffsetY, 0);

        // Optional: Recompute box and log after translation for verification
        textGeom.computeBoundingBox();
        console.log(
          "[Export Text] After Manual X,Y Centering (Z not re-centered):"
        );
        console.log(
          "  - Final BBox min:",
          textGeom.boundingBox.min
            .toArray()
            .map((v) => v.toFixed(3))
            .join(", ")
        );
    ...
    isImportedModelDisplayed,
    importedModel,
    // meshToExportOrScreenshotRef, r3fSceneForExportRef - refs generally stable, direct values in settings used for text
  ]);
